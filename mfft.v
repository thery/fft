...
Lemma fft0 n w : fft n w 0 = 0%R :> {poly R}.
Proof.
elim: n w => /= [|n IH] w; first by rewrite coefC eqxx polyC0.
apply/polyP => i.
rewrite even_polyC odd_polyC polyC0 coef_poly !IH !coefC !if_same.
by rewrite add0r mul0r if_same.
Qed.
...
Definition poly_mul (p1 p2 : {poly R}) := 
  (\poly_(i < minn (size p1) (size p2)) (p1`_i * p2`_i))%R.

Lemma poly_mu0l (p : {poly R}) : poly_mul 0 p = 0.
Proof. by rewrite /poly_mul size_polyC eqxx min0n unlock. Qed.

Lemma poly_mul0 (p : {poly R}) : poly_mul p 0 = 0.
Proof. by rewrite /poly_mul size_polyC eqxx minn0 unlock. Qed.

Lemma coef_poly_mul p q i : ((poly_mul p q)`_i = p`_i * q`_i)%R.
Proof.
rewrite /poly_mul coef_poly; case: leqP => //.
by case: (leqP (size p)); move=> _ /leq_sizeP-> //; rewrite ?(mulr0, mul0r).
Qed.

Lemma poly_mulaMx a b p q : 
  poly_mul (p * 'X + a%:P) (q * 'X + b%:P) = poly_mul p q * 'X + (a * b)%:P.
Proof.
apply/polyP => i; rewrite !(coefE, coef_poly_mul).
case: i => /= [|i]; rewrite !(addr0, add0r) //.
rewrite leq_min.
case: (leqP (size p) i) => /= [/leq_sizeP->//|iLp]; first by rewrite mul0r.
by case: (leqP (size q) i) => [/leq_sizeP->//|//]; rewrite mulr0.
Qed.
...
Lemma ifft0 n w : ifft n w 0 = 0.
Proof. by rewrite /ifft fft0 mulr0. Qed.
...
Definition fft_mul n w (p1 p2 : {poly F}) := 
  let p1' :=  fft n w p1 in
  let p2' :=  fft n w p2 in
  ifft n w (poly_mul p1' p2').

Lemma fft_mu0l n w p : fft_mul n w 0 p = 0%R.
Proof. by rewrite /fft_mul fft0 poly_mu0l ifft0. Qed.

Lemma fft_mul0 n w p : fft_mul n w p 0 = 0%R.
Proof. by rewrite /fft_mul fft0 poly_mul0 ifft0. Qed.

Lemma fft_mul_correct n (p1 p2 : {poly F}) w :
  (2%:R != 0 :> F)%R -> ((size p1 + size p2).-1  <= 2 ^ n)%N -> 
  ((2 ^ n).-primitive_root w)%R ->
  fft_mul n w p1 p2 = (p1 * p2)%R.
Proof.
move=> char2 sL wE.
have [->|/eqP p1_neq0] := p1 =P 0%R; first by rewrite mul0r fft_mu0l.
rewrite -size_poly_leq0 -ltnNge in p1_neq0.
have [->|/eqP p2_neq0] := p2 =P 0%R; first by rewrite mulr0 fft_mul0.
rewrite -size_poly_leq0 -ltnNge in p2_neq0.
rewrite /fft_mul.
...
- by case: (size p1) p1_neq0 sL => //= k _ /(leq_trans _); apply;
     apply: leq_addl.
- case: (size p2) p2_neq0 sL => //= k _ /(leq_trans _); apply.
  by rewrite addnS; apply: leq_addr.
case: leqP; first by rewrite mul0r.
by rewrite hornerE.
Qed.
